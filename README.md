...
<img width="1209" height="812" alt="시스템 아키텍처" src="https://github.com/user-attachments/assets/f07bbb29-fa65-4fca-94fc-6facb23e17bf" />
...
구매나 결제 로직은 데이터 일관성 확보가 최우선이라 판단 &rarr; **비관적 락** 적용

#### 성과 및 결과

- 락을 적용하기 전에는 100건의 동시 요청에서 **2건**의 구매 발생
- 락을 적용한 후에는 100건의 동시 요청에서 **1건**의 구매 발생
...
- 특히 상품 목록은 사용자 진입 시마다 반복적으로 호출되며 결과적으로 다음 문제가 발생
    - DB 쿼리 부하 증가
    - 응답 지연(Latency)
- 실사용자 테스트 당시 **약 70명의 사용자, 50개의 상품 게시물**이 존재하던 상황에서 모바일 데이터 상품과 와이파이 상품 각각 최대 **780ms, 2.39s** 의 응답
  지연이 발생
- 또한 거리 기준 오름차순 정렬 결과는 사용자의 위도/경도에 따라 매번 달라지기 때문에 일반적인 의미에서의 캐싱(“모든 사용자에 대해 한 번만 계산 후 계속 재사용”)은 읽기
  비용(네트워크+메모리 접근)조차 더 크거나 같을 수 있다고 판단
...
<img width="644" src="https://github.com/user-attachments/assets/0d0497a4-fdc5-46a0-b6e2-702cb33b39f1" />

<img width="627" src="https://github.com/user-attachments/assets/71a677be-9729-402c-a7a0-9f10fc49520e" />
...
- 1차 개발에서 단일 서버 환경에서 인메모리 방식의 WebSocket STOMP 기반 채팅 시스템 구현
  ...
  &rarr; 오토스케일링에 의해 스케일 아웃 시, **서로 다른 서버로 트래픽이 나뉘어 사용자들 간의 채팅이 불가능한 문제 발생**
...
- 사용자가 원하는 `dataAmount` 만큼의 모바일 데이터를 **최저 가격**으로 구매하도록 하는 탐색 알고리즘
- 그리디 알고리즘을 변형하여 적용
...
<img width="800" src="https://github.com/user-attachments/assets/e4116f2d-f9c4-4a87-93f0-81e9311e5711" />
...
<img width="928" height="344" alt="Image" src="https://github.com/user-attachments/assets/17159590-3f40-4a68-a97b-0c3def8de3a8" />
...
- [🔒 비관락 도입으로 상품 구매, 캐시 결제 동시성 처리](https://github.com/URECA-Dapanda/dapanda-backend/wiki/%F0%9F%94%92-%EB%B9%84%EA%B4%80%EB%9D%BD-%EB%8F%84%EC%9E%85%EC%9C%BC%EB%A1%9C-%EC%83%81%ED%92%88-%EA%B5%AC%EB%A7%A4,-%EC%BA%90%EC%8B%9C-%EA%B2%B0%EC%A0%9C-%EB%8F%99%EC%8B%9C%EC%84%B1-%EC%B2%98%EB%A6%AC)
- [💨 Redis 캐싱을 통한 조회 성능 개선](https://github.com/URECA-Dapanda/dapanda-backend/wiki/%F0%9F%92%A8-Redis-%EC%BA%90%EC%8B%B1%EC%9D%84-%ED%86%B5%ED%95%9C-%EC%A1%B0%ED%9A%8C-%EC%84%B1%EB%8A%A5-%EA%B0%9C%EC%84%A0)
- [💬 채팅 시스템 개발](https://github.com/URECA-Dapanda/dapanda-backend/wiki/%F0%9F%92%AC-%EC%B1%84%ED%8C%85-%EC%8B%9C%EC%8A%A4%ED%85%9C-%EA%B0%9C%EB%B0%9C)
- [🤖 데이터 자투리 구매 알고리즘](https://github.com/URECA-Dapanda/dapanda-backend/wiki/%F0%9F%A4%96-%EB%8D%B0%EC%9D%B4%ED%84%B0-%EC%9E%90%ED%88%AC%EB%A6%AC-%EA%B5%AC%EB%A7%A4-%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98)
- [🚨 모니터링 시스템 도입 (feat. Slack 알림 연동)](https://github.com/URECA-Dapanda/dapanda-backend/wiki/%F0%9F%9A%A8-%EB%AA%A8%EB%8B%88%ED%84%B0%EB%A7%81-%EC%8B%9C%EC%8A%A4%ED%85%9C-%EB%8F%84%EC%9E%85-(feat.-Slack-%EC%95%8C%EB%A6%BC-%EC%97%B0%EB%8F%99))
- [🛠️ 기술 스택 선택 이유](https://github.com/URECA-Dapanda/dapanda-backend/wiki/%F0%9F%9B%A0%EF%B8%8F-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D-%EC%84%A0%EC%A0%95-%EC%9D%B4%EC%9C%A0)
- [🚀 인프라 설계](https://github.com/URECA-Dapanda/dapanda-backend/wiki/%F0%9F%9A%80-%EC%9D%B8%ED%94%84%EB%9D%BC)
- [📑 Team Convention](https://github.com/URECA-Dapanda/dapanda-backend/wiki/%F0%9F%93%91-Team-Convention)
